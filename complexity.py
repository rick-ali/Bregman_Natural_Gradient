...
def generate_test_space(n):
    original_tensor = torch.tensor([[0.], [1.]])
    for i in range(n-1):
        zero_appended = torch.cat((original_tensor, torch.zeros(original_tensor.size(0), 1)), dim=1)
        one_appended = torch.cat((original_tensor, torch.ones(original_tensor.size(0), 1)), dim=1)
        original_tensor = torch.cat((zero_appended, one_appended), dim=0)
